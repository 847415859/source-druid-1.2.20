...
public abstract class DruidAbstractDataSource extends WrapperAdapter implements DruidAbstractDataSourceMBean, DataSource, DataSourceProxy, Serializable {
    ...
    // 表示连接池中物理连接的超时时间，单位是毫秒。如果某个连接在phyTimeoutMillis指定的时间内没有使用，连接池会将这个连接关闭。这可以防止连接长时间闲置，从而消耗资源。
    protected volatile long phyTimeoutMillis = DEFAULT_PHY_TIMEOUT_MILLIS;
    protected volatile long phyMaxUseCount = -1;
    // 用于指定是否启用废弃连接的自动回收。如果启用，当一个连接长时间没有被使用，并且达到了特定的超时时间，连接将被回收。这有助于防止连接泄露，即长时间被占用但不使用的数据库连接。
    protected volatile boolean removeAbandoned;
    // removeAbandoned 能够移除的设置的超时时间。
